...
win_init_fontfamily(HDC dc, int findex)
{
  ...
  ff->errch = 0;
  dw_font_changed();  // drop cached IDWriteFontFace for this family
...
static bool use_dwtext;  // runtime FontRender=dwrite text path (not the use_dwrite macro)
...
text_out_start(HDC hdc, LPCWSTR psz, int cch, int *dxs)
{
  ...
  if (use_dwtext) {
    if (dw_text_start(hdc, (wchar *)psz, cch, dxs))
      return;
    use_dwtext = false;  // shaping failed → plain ExtTextOutW
  }
  if (!use_uniscribe)
    return;
...
text_out(HDC hdc, int x, int y, UINT fuOptions, RECT *prc, LPCWSTR psz, int cch, int *dxs)
{
  ...
  if (use_dwtext) {
    if (dw_text_out(hdc, x, y, fuOptions, prc, (wchar *)psz, cch, dxs))
      return;
    use_dwtext = false;  // identity/shape miss: fall back
  }
  if (use_uniscribe)
...
text_out_end()
{
  dw_text_end();  // no-op if no run; always free shaped resources
  if (use_uniscribe)
...
#define TATTR_TRIPLE 0x0080000000000000u
  ...
  if (cfg.smooth_blink_attr != ANIM_NONE && term.blink_is_real && term.has_focus
...
#warning now passed as a parameter to avoid redundant checking
 ...
 /* Painting box */
  ...
  /* Uniscribe / DirectWrite handling */
  use_uniscribe = cfg.font_render == FR_UNISCRIBE && !has_rtl;
  use_dwtext = cfg.font_render == FR_DWRITE && !has_rtl;
  if (combining_double) {
    use_dwtext = false;
    use_uniscribe = false;
  }
...
#define selfdrawn() (vt52fraction || boxpower || dectcs || boxcoded)
 ...
 /* Coordinate transformation per character */
  ...
  RECT box_, box2_;
  if (wscale != 100) {
    use_dwtext = false;  // DWrite advances are not world-transform aware here
  ...
  for (int xoff0 = 0; xoff0 < xwidth; xoff0++) {
    ...
    if ((combining || combining_double) && !has_sea) {
      ...
      for (int i = ulen; i < len; i += ulen) {
        ...
        // does not work with Uniscribe or DWrite shaping
        use_uniscribe = false;
        use_dwtext = false;
...
# endif
    ...
    int dx = 0;
    use_dwtext = false;  // measure via GDI/Uniscribe pixel path, not DWrite shape
